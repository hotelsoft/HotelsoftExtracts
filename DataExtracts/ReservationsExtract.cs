...
namespace DataExtracts
{
	public class ReservationsExtract
	{
		...
		SELECT 
			...
			, G.PCODE AS RATE_REQ, G.KAT AS ROOMTYPE, G.ANZ AS ROOMS, G.MARKET AS MARKETCD, M.TEXT AS MARKETNAME, G.ALOTM AS  BLOCKID, '' AS GROUPNAME
			...
			using (var csv = new CsvWriter(streamWriter, config))
			{
				...
				while (rd.Read())
				{
					...
					var record = new ReservationRecord
					{
						...
						RateCode = rd.GetString("RATE_REQ"),
